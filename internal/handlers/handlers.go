...
// AdminNewReservations shows all new reservarions in admin tool
...
func (m *Repository) AdminReservationsCalendar(w http.ResponseWriter, r *http.Request) {
	// assume that there is no month/year specifed
	now := time.Now()
	...
	for _, x := range rooms {
		...
		for name, value := range curMap {
			...
			if val, ok := curMap[name]; ok {
				// only pay attension to values > 0, and that are not in the form post
